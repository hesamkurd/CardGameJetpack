...
import ir.mamhesam.cardgamejetpack.ui.theme.EXTRA_SMALL_PADDING
...
fun CalculateStar(rating : Double) : Map<String , Int>
{
    ...
    LaunchedEffect(key1 = rating) {
        val (firstNumber , secondNumber) = rating
            .toString()
            .split(".")
            .map { it.toInt() }
        ...
        } else
        {
            Log.d(
                "Rating Widget" ,
                "CalculateStar: Invalid rating number. "
            )
        }
    }
    emptyStar = maxStar - (filledStar + halfFilledStar)
    return mapOf(
        "filledStar" to filledStar ,
